...
Transaction v1 support for Agave 4.2, raising the maximum transaction size from 1,232 to 4,096 bytes.

### Added

- **Transaction v1 support.** `createSmartTransaction` (and everything built on it, including the Sender path) now accepts `version: 1`, unlocking the 4,096-byte limit from [SIMD-0296](https://github.com/solana-foundation/solana-improvement-documents/blob/main/proposals/0296-larger-transactions.md). The compute-unit limit and priority fee are written to the v1 header config rather than emitted as `ComputeBudgetProgram` instructions, which [SIMD-0385](https://github.com/solana-foundation/solana-improvement-documents/blob/main/proposals/0385-transaction-v1.md) makes no-ops. A v1 transaction is smaller on the wire than the v0 equivalent before you use any of the extra room. Legacy and v0 are unchanged and remain the default.
- **`priorityFeeLamports` on `CreateSmartTxResult`.** Version 1 pays a total priority fee in lamports rather than a per-compute-unit rate, so the result now reports both. On legacy and v0 it is the equivalent total implied by the rate over the compute-unit limit.
- **`priorityFeeLamportsCap` on `CreateSmartTxInput`**, to budget against absolute priority spend. Applied by clamping the per-CU rate, so it constrains every version.
- **Version 1 validation.** Address lookup tables are rejected on v1 (the format drops them, and `@solana/kit` would otherwise silently inline every address), and the transaction size limit is checked before signing so an oversized transaction fails locally rather than at a wallet prompt or on submission.
- `SupportedTxVersion` is now exported, and `CreateTxMessageInput` / `createTxMessage` are generic over the version so a v1 caller receives a message typed as v1.

### Fixed

- **Compute-unit estimation on version 1.** The estimator from `@solana-program/compute-budget` raises the simulation limit by appending a `SetComputeUnitLimit` instruction, which is a no-op on v1 — every v1 estimate would have simulated against the default limit and failed on compute exhaustion for anything non-trivial. Now uses `@solana/kit`'s estimator, which sets the limit through a version-aware helper. Behaviour on legacy and v0 is unchanged.
- The compute-unit estimator is built on first use rather than at client construction, so creating a client no longer does estimator setup for consumers who never send a transaction.
- **Fractional priority fee estimates no longer crash.** `getPriorityFeeEstimate` can return a fractional microLamports-per-CU value, which the compute-budget instruction's `u64` encoder rejected with `The number 4032.3712 cannot be converted to a BigInt because it is not an integer`. The rate is now floored to a whole microLamport before use. Rounding down rather than to nearest also keeps `priorityFeeCap` and `priorityFeeLamportsCap` true ceilings.

### Notes for upgraders

- No peer dependency changes; the existing `@solana/kit` v6 range already supports v1.
- `CreateSmartTxResult` gained a required `priorityFeeLamports` field. Reading results is unaffected, but code that *constructs* a `CreateSmartTxResult` — typically a test mock or a custom `CreateSmartTransactionFn` — needs the new field.
