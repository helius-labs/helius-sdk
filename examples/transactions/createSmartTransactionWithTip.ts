...
(async () => {
  ...
  try {
    ...
    const smart = await helius.tx.createSmartTransactionWithTip({
      signers: [feePayerSigner], // First signer = fee payer
      instructions: [transferIx], // No compute-budget ixs — SDK will prepend them
      tipAmount: 2_000, // Lamports to tip (default = 1000)
      version: 0,
    ...
    console.log("— createSmartTransactionWithTip() result —");
